        ...
        promises.push(
          ftp.get(article.thumbnailImage)
            .then(stream => {
              ...
              let imageAbsolutePath = path.resolve(`.${imagePath}`)
              let imageAblsoluteDir = path.dirname(imageAbsolutePath)
              return fs.ensureDir(imageAblsoluteDir)
                .then(() => {
                  return new Promise((resolve, reject) => {
                    // writing the images in uploads/news directory
                    article.thumbnailImage = imagePath
                    stream.pipe(fs.createWriteStream(imageAbsolutePath))
                    stream.once('end', resolve())
                    stream.once('error', reject({message: 'Stream error'}))
                  })
                })
            })
        )
